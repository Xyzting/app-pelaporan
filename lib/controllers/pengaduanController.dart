...
class PengaduanController extends GetxController{
  ...
  getData () async {
    try{
      data.value = [];
      loading.value = true;
      var response = await fetch.get(Uri.parse("http://localhost:2222/api/pengaduan"));
      loading.value = false;
  ...
  createData (Map data) async {
    ...
    final url = Uri.parse('http://localhost:2222/api/pengaduan');
  ...
  updateData (Map data,id) async {
    ...
    final url = Uri.parse('http://localhost:2222/api/pengaduan/${id}');
  ...
  destroyData (id) async {
    try{
      ...
      var response = await fetch.delete(
        Uri.parse("http://localhost:2222/api/pengaduan/${id}"),
      );
      loading.value = false;
