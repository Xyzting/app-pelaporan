...
class TanggapanController extends GetxController {
  ...
  getData () async {
    try{
      data.value = [];
      loading.value = true;
      var response = await fetch.get(Uri.parse("http://localhost:2222/api/tanggapan")); // proses
      loading.value = false;
  ...
  createData (data) async {
    try{
      ...
      var response = await fetch.post(
        Uri.parse("http://localhost:2222/api/tanggapan"),
  ...
  updateData (data,id) async {
    try{
      ...
      var response = await fetch.put(
        Uri.parse("http://localhost:2222/api/tanggapan/${id}"),
        body:jsonEncode(data),
        headers: {
          'Content-Type': 'application/json',
        },
