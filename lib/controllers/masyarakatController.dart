...
class MasyarakatController extends GetxController {
  ...
  getData () async {
    try{
      data.value = [];
      loading.value = true;
      var response = await fetch.get(Uri.parse("http://localhost:2222/api/masyarakat")); // proses
      loading.value = false;
  ...
  createData (String nik,String nama,String username,String password,String telp) async {
    try{
      ...
      var response = await fetch.post(
        Uri.parse("http://localhost:2222/api/masyarakat"),
        body:jsonEncode({
          "nik":nik,
          "nama":nama,
          "username":username,
  ...
  updateData (nik,String nama,String username,String telp) async {
    try{
      ...
      var response = await fetch.put(
        Uri.parse("http://localhost:2222/api/masyarakat/${nik}"),
        body:jsonEncode({
          "nama":nama,
          "username":username,
          "telp":telp,
