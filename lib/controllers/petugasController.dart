...
class PetugasController extends GetxController {
  ...
  getData () async {
    try{
      data.value = [];
      loading.value = true;
      var response = await fetch.get(Uri.parse("http://localhost:2222/api/petugas"));
      loading.value = false;
  ...
  createData (String nama_petugas,String username,String password,String telp, String level) async {
    try{
      loading.value = true;
      var response = await fetch.post(
        Uri.parse("http://localhost:2222/api/petugas"),
        body:jsonEncode({
  ...
  updateData (id,String nama_petugas,String username,String telp) async {
    try{
      ...
      var response = await fetch.put(
        Uri.parse("http://localhost:2222/api/petugas/${id}"),
        body:jsonEncode({
          "nama_petugas" : nama_petugas,
          "username" : username,
          "telp" : telp
